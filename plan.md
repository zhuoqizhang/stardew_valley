...
> 记录时间点：2026-09-11。本节记录啤酒/淡啤酒每日市场价格的模拟定价模型——经过多轮离线模拟调参后
> 定稿，本次落地为独立纯逻辑类 `PriceModel.cs`（零 SMAPI/Game1 依赖，风格仿照 `SettlementMath.cs`）。
> **接入状态（过渡期）**：`PriceModel` 目前只在每日 `DayStarted` 时算出当日模拟价并写入价格历史，
> 供以后做折线图使用；`ContractManager.GetMarketPrice`（违约结算取价的入口）暂时**不**读取这套模拟
> 价格，而是直接返回游戏原生真实售价，直到模拟结果经过验证、由用户明确下达指令后才切换过去（详见
> 6.6 节）。这么做是为了避免在新定价系统还没跑通之前，违约罚金计算用着一半新一半旧的价格逻辑。
...
### 6.6 与 GetMarketPrice 的接入状态（过渡期说明）

> 记录时间点：2026-09-11。

`ContractManager.GetMarketPrice` 是违约结算（5.4 节差价罚金）取价的唯一入口。在 `PriceModel`
的每日更新逻辑经过测试验证、正式接入之前，`GetMarketPrice` 直接查询 `ItemRegistry` 返回该物品的
游戏原生真实售价，不做任何模拟计算——这是刻意的过渡期行为，避免违约罚金计算用着一半新（模拟）
一半旧（原生价）的价格逻辑，也避免在模拟结果还没验证之前就影响玩家的实际金币结算。

`PriceModel` 目前只接在 `ModEntry` 的 `DayStarted` 事件上，每天算出啤酒/淡啤酒的模拟价格并追加进
价格历史（随存档持久化，见 `MarketPriceSaveData`），供以后做折线图使用；这条更新链路**不会**
反过来影响 `GetMarketPrice` 的返回值。等模拟结果经过验证，由用户另行下达指令后，才会把
`GetMarketPrice` 切换到读取 `PriceModel` 算出来的价格。
...
5. ~~违约结算（`DayEnding`）~~ ✅ 已完成（`SettlementMath.cs`）；`GetMarketPrice` 目前是过渡期实现，
   直接返回游戏原生真实售价，尚未接入 `PriceModel` 模拟价格（见 6.6 节）
6. ~~存档持久化~~ ✅ 已完成（合约数据见 `FuturesContractSaveData.cs`；价格历史见 `MarketPriceSaveData.cs`）
7. 折线图 / Tab 视觉打磨 —— 未开始，明确推迟到功能验证完成之后
8. 市场定价模型接入 —— `PriceModel.cs` 分层公式与每日价格历史记录已实现（见第 6 节），
   但尚未接入 `GetMarketPrice`；等模拟结果经验证后由用户另行下达接入指令
